...
class LookupService(object):
    ...
    def GetModels():
        ...
        return urllib.urlopen(service.serviceUri).read().split('\r\n') # defines a tuple on the fly
    ...
    def _getData(self, phrase, operation, args=None):
        ...
        urlAddr=self.serviceUri+self.model+'/'+operation+'?p='+urllib.quote(phrase)+'&u='+self.token
        if (args is not None):
            for k,v in args.iteritems():
                urlAddr = urlAddr + '&' + k + '=' + urllib.quote(str(v))
        return urllib2.urlopen(urlAddr).read()

    def _getMultipleData(self, phrases, operation):
        if (self.model is None):
            raise ValueError('model must be specified, either as an argument to the LookupService constructor, or as an environment variable named NGRAM_MODEL')
        urlAddr=self.serviceUri+self.model+'/'+operation+'?p='+'&u='+self.token
        urlPhrase = ""
        for s in phrases:
            urlPhrase += s + '\n'
        ''' remove the last '\n' '''
        urlPhrase = urlPhrase[:-1]
        req = urllib2.Request(urlAddr, urlPhrase)
        return urllib2.urlopen(req).read()

    def _getProbabilityData(self, phrase, operation):
        return float(self._getData(phrase, operation))

    def _getProbabilitiesData(self, phrase, operation):
        return self._getMultipleData(phrase, operation)
    ...
    def GetJointProbabilities(self, phrase):
        return self._getProbabilitiesData(phrase,'jp')

    def GetConditionalProbabilities(self, phrase):
        return self._getProbabilitiesData(phrase,'cp')
